...
cpf = 0  # lembrar de remover isso antes do código final, isso é só para teste
...
def validacaocpf():
    ...
    if len(cpf) != 11 or not cpf.isdigit() or len(set(cpf)) == 1:
        ...
        for i in range(9):
            # Aqui nesta linha pegará os 9 primeiros digitos e multiplicar cada numero pelo descrescente dos digitos(coloquei a variavel como 'dg')
            soma1 += digitoscpf[i]*dg
            dg -= 1  # Aqui o digito da multiplicação vai diminuindo para que não multiplique todos os 9 números por 10

        # Aqui calcula-se o resto desta soma para descobrir o primeiro numero verificador
        resto1 = soma1 % 11
        if resto1 < 2:
            resto1 = 0
        else:
        ...
        dg2 = 11
        for i in range(10):
            # É a mesma coisa, porém desta vez a multiplicação começa com '11' invés de '10'
            soma2 += digitoscpf[i]*dg2
            dg2 -= 1
